...
class FtEncoder(nn.Module):
  def __init__(self, z_dim, initailize_weights=True):
    ...
    input size: n*8*6 (8 is time step)
    """
    super().__init__()
    self.z_dim = z_dim
...
class ImageEncoder(nn.Module):
  def __init__(self, z_dim, initailize_weights=True):
    ...
    self.img_conv4 = conv2d(64, 128, stride=2)
    self.img_conv5 = conv2d(128, 128, stride=2)
    self.img_conv6 = conv2d(128, self.z_dim, stride=2)
    self.img_encoder = nn.Linear(4 * self.z_dim, self.z_dim)
...
class DepthmapEncoder(nn.Module):
  def __init__(self, z_dim, initailize_weights=True):
    ...
    self.depth_encoder = nn.Linear(4 * self.z_dim, self.z_dim)
    self.flatten = Flatten()
  ...
  def forward(self, depth):
    ...
    depth_out = self.depth_encoder(flattened)
...
class FusionNet(nn.Module):
  ...
  def forward(self, concat_z):
    return self.fusion_net(concat_z)
